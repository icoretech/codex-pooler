defmodule CodexPooler.Upstreams.PreparedImportPersistenceTest do
  ...
  # Registered before the commit, never scoped in `try/after`: the stale-race case runs both
  # writers in linked tasks, so an assertion failing in one kills the test process before an
  # enclosing `after` runs, and the ExUnit timeout does the same to every case here. The committed
  # pool and identities would then outlive the test, and the absolute identity counts asserted in
  # this file are exactly what a leaked identity breaks.
  defp committed_fixture! do
    ...
    %{user: user} =
      committed_bootstrap_owner_fixture!(%{
        "email" => "prepared-persistence-#{suffix}@example.com"
      })
