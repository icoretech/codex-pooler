defmodule CodexPooler.Gateway.Metadata.CodexModelDecodeContractTest do
  ...
  # were checked against the released 0.156.1, 0.157.0 and 0.157.1 decoders (`codex debug
  ...
  describe "violations/1" do
    ...
    test "states the verified client window" do
      assert CodexModelDecodeContract.verified_range() == {{0, 154, 0}, {0, 157, 1}}

      for version <- [{0, 154, 0}, {0, 155, 1}, {0, 156, 0}, {0, 156, 1}, {0, 157, 0}, {0, 157, 1}] do
        assert CodexModelDecodeContract.verified_version?(version), inspect(version)
      end

      for version <- [{0, 153, 4}, {0, 157, 2}, {0, 158, 0}, {1, 0, 0}] do
