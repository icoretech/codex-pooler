defmodule CodexPooler.Gateway.Metadata.CatalogRepresentationTest do
  ...
  describe "version table" do
    test "clients whose every build prefers the instructions template get the template-only entry" do
      for version <- ["0.148.0", "0.153.4", "0.157.2", "0.158.0", "0.200.0", "1.0.0", "0.148.0-alpha.1"] do
    ...
    # CodexModelDecodeContract mirrors (0.154.0 through 0.157.1) also lose the
    ...
    test "clients inside the verified decode window get the decode-checked template-only entry" do
      for version <- ["0.154.0", "0.154.0-alpha.6.2", "0.155.0", "0.155.1", "0.156.0", "0.156.0-alpha.18", "0.156.1", "0.157.0", "0.157.0-alpha.1", "0.157.0-alpha.11.1", "0.157.1"] do
  ...
  describe "for_user_agent/1" do
    test "reads the package version Codex puts after its originator" do
      ...
      for user_agent <- ["codex_cli_rs/0.148.0", "codex_cli_rs/0.153.4 (Linux; x86_64)", "codex_exec/0.157.2 (Linux; aarch64)"] do
