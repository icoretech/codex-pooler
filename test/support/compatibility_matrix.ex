defmodule CodexPooler.CompatibilityMatrix do
  ...
  @features [
    ...
    %{
      ...
      contract:
        "backend Responses HTTP and websocket response.create lower and remove encrypted markers only for ordinary top-level non-strict function tool schemas while preserving every decoded top-level namespace tool term exactly; public /v1 Responses HTTP and websocket recursively lower nested namespace function tools before local validation or upstream dispatch; lowering converts boolean schemas and const values into supported schema shapes, drops unsupported JSON Schema keywords, preserves supported refs/definitions/combinators recursively, and for strict function parameters infers only a missing or invalid scalar type when object or array structural keys unambiguously identify one type; invalid or ambiguous type arrays and schemas with both object and array structural evidence remain rejected, and never weakens strict function tools or strict structured-output schemas"
    },
    %{
  ...
  @fixtures %{
    ...
    function_tool_schema_lowering: %{
      ...
      schema_repairs: [
        ...
        "infer_missing_or_invalid_scalar_object_type_from_unambiguous_structure",
        "infer_missing_or_invalid_scalar_array_type_from_unambiguous_structure",
        "preserve_invalid_or_ambiguous_type_arrays_for_rejection",
        "reject_mixed_object_array_structural_evidence",
    ...
    v1_supported_surface: %{
      ...
      chat_custom_tools: %{executable: false, supported: false},
      instruction_lifting: %{
