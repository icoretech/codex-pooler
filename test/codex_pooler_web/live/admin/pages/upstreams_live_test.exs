defmodule CodexPoolerWeb.Admin.UpstreamsLiveTest do
  ...
  test "links upstream account through browser OAuth dialog without rendering token secrets", %{
    ...
    assert has_element?(
             view,
             ~s(#oauth-link-authorization-url[target="_blank"][rel="noopener noreferrer"])
           )

    assert has_element?(
             view,
             ~s(#oauth-link-authorization-url-copy[type="button"][phx-hook="ClipboardCopy"][data-copy-text="#{authorization_url}"][aria-label="Copy OpenAI authorization URL"])
           )

    assert has_element?(
             view,
             "#oauth-link-authorization-url-copy .copy-icon.hero-clipboard-document"
           )

    assert has_element?(view, "#oauth-link-authorization-url-copy [data-copy-label].sr-only")
  ...
  test "links upstream account through device OAuth polling without rendering hidden provider values",
    ...
    pending_provider_value = "raw-admin-device-pending-must-not-leak"
    ...
    assert has_element?(
             view,
             ~s(#oauth-link-device-code-copy[type="button"][phx-hook="ClipboardCopy"][data-copy-text="CODE-UI"][aria-label="Copy device code"])
           )

    assert has_element?(
             view,
             "#oauth-link-device-code-copy .copy-icon.hero-clipboard-document"
           )

    assert has_element?(view, "#oauth-link-device-code-copy [data-copy-label].sr-only")
    ...
    assert has_element?(
             view,
             ~s(#oauth-link-device-verification-url[href="#{verification_url}"][target="_blank"][rel="noopener noreferrer"])
           )

    assert has_element?(
             view,
             ~s(#oauth-link-device-verification-url-copy[type="button"][phx-hook="ClipboardCopy"][data-copy-text="#{verification_url}"][aria-label="Copy device verification URL"])
           )

    assert has_element?(
             view,
             "#oauth-link-device-verification-url-copy .copy-icon.hero-clipboard-document"
           )

    assert has_element?(
             view,
             "#oauth-link-device-verification-url-copy [data-copy-label].sr-only"
           )
