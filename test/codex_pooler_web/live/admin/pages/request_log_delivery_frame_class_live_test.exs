defmodule CodexPoolerWeb.Admin.RequestLogDeliveryFrameClassLiveTest do
  ...
  # served. The admin request-log drawer shows it next to the other receipt
  # fields. The receipt here is the one a real socket records: a native turn
  # through the real listener, owner forwarding off (direct task), the fake
  # provider held at a frame barrier right after `response.output_item.added`,
  # and the client closing its connection there.
  ...
  test "the request-log drawer shows the highest frame class a native socket pushed before the client left", %{conn: conn} do
    ...
    socket_conn = receive_until!(socket_conn, websocket, ref, "response.output_item.added")
