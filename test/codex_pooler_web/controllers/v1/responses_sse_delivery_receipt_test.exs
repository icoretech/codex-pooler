defmodule CodexPoolerWeb.V1.ResponsesSseDeliveryReceiptTest do
  ...
  @prompt_sentinel "v1-sse-receipt-prompt-sentinel"
  ...
  test "completed public SSE turn records a delivered downstream receipt on the attempt", %{
    conn: conn
  } do
    response_id = "resp_v1_receipt_completed"
    ...
    {conn, logs} =
      with_log([level: :info], fn ->
        conn |> auth(setup) |> post(@public_path, stream_payload(setup))
      end)
  ...
  test "client disconnect before the terminal records an aborted downstream receipt" do
    upstream =
      start_upstream(
        ...
        FakeUpstream.strict_sequence([
          expect_dispatch(
            FakeUpstream.sse_stream([created_event("resp_v1_receipt_abort"), delta_event()],
              done: false
            )
          )
        ])
    ...
    # The direct gateway call sits behind the controller's input coercion, so
    # the payload carries the already-coerced message array.
    payload =
      Map.put(stream_payload(setup), "input", [
        %{
          "type" => "message",
          "role" => "user",
          "content" => [%{"type" => "input_text", "text" => @prompt_sentinel}]
        }
      ])
    ...
    assert_metadata_only!(request, attempt, logs, "resp_v1_receipt_abort")
  end

  test "provider response.failed after visible output records a response.failed terminal class",
       %{conn: conn} do
    response_id = "resp_v1_receipt_failed"
