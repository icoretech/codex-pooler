defmodule CodexPoolerWeb.Runtime.BackendCodexWebsocket.PrevisibleDirectLateAnswerTest do
  ...
  # rollups count it once (findings#232 row 232-173).
  @tag slow: "waits out the closing socket's 250 ms response-task drain and then a late provider answer"
  test "a provider answer after the client left a turn it had shown output corrects the interrupt's settlement instead of adding one" do
    ...
    upstream =
      start_upstream(
        # provenance: observed findings#232 row 232-173 (owner forwarding off, client gone after a visible delta, provider answering afterwards)
        FakeUpstream.strict_sequence([
          strict_native_request(
            1,
            FakeUpstream.barrier_websocket_frames(
              [
                ...
                CodexPooler.JSON.encode!(%{"type" => "response.output_text.delta", "item_id" => "msg_direct_postvisible", "output_index" => 0, "content_index" => 0, "delta" => "partial"}),
                CodexPooler.JSON.encode!(%{
                  "type" => "response.completed",
    ...
    conn = await_client_frame!(conn, websocket, ref, "response.output_text.delta")
