defmodule CodexPoolerWeb.Runtime.BackendCodexCatalogDecodeContractTest do
  ...
  # inside the verified decode window (0.154.0 through 0.157.1) is served the
  ...
  @user_agent "codex_cli_rs/0.157.1 (Mac OS 26.0.0; arm64) xterm-256color"
  ...
  test "a client inside the verified window is served every entry but the one it cannot decode", %{conn: conn} do
    ...
    for path <- ["/backend-api/codex/models", "/backend-api/codex/v1/models"],
        version <- ["0.156.1", "0.157.0", "0.157.0-alpha.11.1", "0.157.1"] do
      log =
        capture_log(fn ->
    ...
    for version <- ["0.157.2", "0.158.0", "0.153.4", "0.146.1", ""] do
  ...
  test "a turn from a client inside the window names the ETag of the checked catalog", %{conn: conn} do
    ...
    capture_log(fn ->
      ...
      unchecked =
        conn |> recycle() |> auth(setup) |> put_req_header("user-agent", user_agent("0.157.2")) |> get("/backend-api/codex/models", %{"client_version" => "0.157.2"}) |> get_resp_header("etag")
  ...
  test "the default gateway fixture model is a decodable catalog entry for every client", %{conn: conn} do
    ...
    log =
      capture_log(fn ->
        for version <- ["0.156.1", "0.154.0", "0.157.0", "0.157.1", "0.146.1"] do
