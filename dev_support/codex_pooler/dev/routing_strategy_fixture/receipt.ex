defmodule CodexPooler.Dev.RoutingStrategyFixture.Receipt do
  @moduledoc false
  ...
  def read(path) do
    with :ok <- reject_symlink(path) do
      case File.read(path) do
        ...
        {:error, _reason} -> {:error, "could not read routing strategy fixture receipt"}
      end
    end
  end
  ...
  def remove!(path) do
    case File.rm(path) do
      :ok -> :ok
      {:error, :enoent} -> :ok
      {:error, _reason} -> raise "could not remove routing strategy fixture receipt"
    end
  end
    ...
    case File.open(lock_path, [:write, :exclusive, :binary]) do
      ...
      {:error, :eexist} ->
        if System.monotonic_time(:millisecond) < deadline do
          ...
          {:error, "routing strategy fixture receipt is busy"}
        end

      {:error, _reason} ->
        {:error, "could not lock routing strategy fixture receipt"}
    end
  end
  ...
  defp decode(body) do
    case CodexPooler.JSON.decode(body) do
      {:ok, %{} = setup} -> {:ok, setup}
      _invalid -> {:error, "routing strategy fixture receipt is invalid"}
    end
  end
  ...
  defp ensure_private_root!(root) do
    case File.lstat(root) do
      {:ok, %File.Stat{type: :symlink}} ->
        raise "routing strategy fixture root cannot be a symlink"

      {:ok, %File.Stat{type: :directory}} ->
        :ok

      {:ok, _stat} ->
        raise "routing strategy fixture root must be a directory"

      {:error, :enoent} ->
        File.mkdir_p!(root)

      {:error, _reason} ->
        raise "could not inspect routing strategy fixture root"
    end
  ...
  defp reject_symlink(path) do
    case File.lstat(path) do
      {:ok, %File.Stat{type: :symlink}} ->
        {:error, "routing strategy fixture receipt cannot be a symlink"}
      ...
      {:error, _reason} ->
        {:error, "could not inspect routing strategy fixture receipt"}
    end
  end
