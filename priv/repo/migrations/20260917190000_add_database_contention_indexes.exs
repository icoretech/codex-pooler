defmodule CodexPooler.Repo.Migrations.AddDatabaseContentionIndexes do
  use Ecto.Migration

  @disable_ddl_transaction true

  # The cost summary includes voided settlements, so its index must not carry
  # the recorded-only predicate used by the owner-scoped indexes.
  @indexes [
  ...
  defp drop_unusable_index(name, _state) do
    repo().query!("DROP INDEX CONCURRENTLY public.#{name}", [], log: false, timeout: :infinity)
  end
