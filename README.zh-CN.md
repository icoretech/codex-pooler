...
`228000` 会在 OpenCode 默认 20k 预留之后留下 208k 可用输入 tokens，因此 208k
输入加 64k 输出上限仍在 Codex Pooler 的 272k `gpt-5.5` 窗口内。OpenCode 的
...
family 行为，例如远程压缩、网页搜索/图片可用性，以及 Codex 后端请求压缩。
...
如果你必须为旧模型 catalog 固定 `gpt-5.5`，使用 Codex 自己的字段：
`model_context_window = 272000` 和 `model_auto_compact_token_limit = 244800`。
Codex 会为 turn 预算计算一个有效 95% 窗口，并且不会在普通 `/responses` turn
中发送 OpenAI SDK 风格的输出上限。
...
作为有效运行时预算。`gpt-5.5` OpenAI source catalog 发布 1M 原生窗口、272k
有效上限和 128k 输出预算；显式压缩预留会在长 completion 之前把本地历史控制在
剩余 144k 提示词预算内。

如果你更希望把 Codex Pooler 与 OpenClaw 内置 OpenAI provider 行为分开，可以
改用自定义 provider id，例如 `codex-pooler/gpt-5.5`。这会遵循 OpenClaw 的
...
自动探测可以解析 Pooler 窗口。对 `gpt-5.5` 来说，原始 Codex 窗口仍是 272000，
当前有效公布值是 258400。把 `context_length: 258400` 保留在 Hermes 配置中，
可作为旧 Pooler 部署或过期模型元数据的安全显式覆盖。
...
字段。内置 Pi `gpt-5.5` 条目使用 272k 上下文窗口和 128k 输出预算，因此匹配这些
值可以让 Codex Pooler 自定义 provider 行为与 Pi 内置元数据保持一致。显式压缩
预留会让 Pi 在提示词加长 completion 可能超过 272k 窗口之前进行压缩。
...
`limit.input: 228000` 会在默认 20k 预留后留下 208k 可用输入 tokens；75% 阈值
...
| Full URL | Off |
| Model ID | `gpt-5.5` 或分配到的 Pool 可服务的另一个模型 id |
...
Aider 内置的 `gpt-5.5` 设置定义差异编辑、repo-map 使用和 `reasoning_effort`
等行为标志；它们不会在 `.aider.conf.yml` 中附带上下文/输出限制。如果你需要显式
unknown-model 限制，请使用 Aider 独立的模型元数据 JSON 文件，而不是把不支持的
上下文字段加到主配置中。
...
总结/压缩，因此应把上下文长度保持为 Codex Pooler 的 272k `gpt-5.5` 窗口，而不是
使用直接 OpenAI 1M 元数据。
...
`contextWindow: 272000`、`maxInputTokens: 144000` 和 `maxTokens: 128000`，这样
Cline 的压缩触发器会在 272k Pooler 窗口内为长 completion 留空间。
...
压缩阈值是上下文限制的比例，不是输出预留，因此 `0.52` 会在提示词历史挤占
Codex Pooler 272k `gpt-5.5` 窗口中的 128k completion 空间之前压缩。
...
模型名应保持为 `openai/gpt-5.5`，这样 OpenHands 会选择它的 OpenAI 兼容
...
`GET /v1/models` 可能包含 `context_length`，供会探测 OpenAI 兼容模型列表的
客户端使用，例如 Hermes。官方 OpenAI SDK 请求 API 和 Vercel AI SDK 生成 API
