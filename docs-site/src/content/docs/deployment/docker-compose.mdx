...
For a reproducible install, choose the release image before creating `.env`.
...
export CODEX_POOLER_IMAGE_TAG=0.4.12
...
For an existing Compose install, update the release image first. If you pin
releases, edit `CODEX_POOLER_IMAGE_TAG` in `.env` to the target release tag.
