...
When Codex Pooler serves current model metadata, Codex does not need explicit client-side context overrides. If you must pin `gpt-5.6-terra` before Codex has refreshed backend metadata, use Codex's raw window fields: `model_context_window = 272000` and `model_auto_compact_token_limit = 232560`. Codex computes an effective 95% turn budget, so the client-visible budget is 258400 tokens, and it does not send an OpenAI SDK-style output cap on normal `/responses` turns. For clients that use the narrow OpenAI-compatible model list, `/v1/models.context_length` is the authoritative effective value instead of this native raw field.
