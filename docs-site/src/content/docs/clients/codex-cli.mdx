...
Keep your Pool API key in an environment variable. Don't paste raw keys into `CODEX_HOME/config.toml`.

## Websocket provider for Codex CLI and Desktop

Use the websocket-capable provider for normal Codex backend behavior in Codex CLI and Codex Desktop. Keep the provider id as `codex-pooler-ws`, but keep `name = "OpenAI"` exactly. In current Codex sources, `name` is not just a display label: exact `OpenAI` matching enables OpenAI-family behavior such as remote compaction, web search/image availability, and Codex backend request compression.
...
Both providers use the same Pool API key and the same Codex backend compatibility route in Codex CLI and Codex Desktop. Keep `requires_openai_auth = false` so Codex uses `env_key` for the Pool API key and does not start the ChatGPT/OpenAI account login flow for this provider. Codex Pooler routes each request through Pool policy, account eligibility, limits, session continuity, and request accounting.

When Codex Pooler serves current model metadata, Codex does not need explicit client-side context overrides. If you must pin `gpt-5.6-terra` before Codex has refreshed backend metadata, use Codex's raw window fields: `model_context_window = 272000` and `model_auto_compact_token_limit = 244800`. Codex computes an effective 95% turn budget, so the client-visible budget is 258400 tokens, and it does not send an OpenAI SDK-style output cap on normal `/responses` turns.
