...
When the Pool's `/v1/models` entry provides `context_length`, use that endpoint's effective value as the authoritative `max_tokens` value. For long-context GPT-5.6, OpenAI's catalog defaults to 272000 tokens and permits a native 872000-token opt-in, while Pooler advertises `floor(872000 × 0.95) = 828400`; the explicit 700400 input and 128000 output limits add up to that advertised window.
