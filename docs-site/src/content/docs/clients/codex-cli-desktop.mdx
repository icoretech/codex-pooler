...
- Codex refuses a catalog response larger than 1 MiB and then keeps its bundled catalog. Current Codex model entries are large, typically 40 to 90 KB each, so a Pool that exposes many models can reach that limit.
