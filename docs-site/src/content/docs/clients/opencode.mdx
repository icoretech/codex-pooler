...
OpenCode subtracts its compaction reserve from `limit.input` before declaring the session full. `194400` leaves 174.4k usable input tokens after the default 20k reserve, so 174.4k input plus a 64k output cap stays inside the 258.4k GPT-5.6 window used by these examples. OpenCode caps request output at 32k by default; set `OPENCODE_EXPERIMENTAL_OUTPUT_TOKEN_MAX=64000` only when you want OpenCode to request the full 64k cap. When available, `/v1/models.context_length` is the authoritative effective value for `limit.context`.
