...
The `-low` and `-high` presets also work. Do not use Hermes' `-xhigh` or
`-max` presets through Codex Pooler: the adapter accepts only `auto`, `low`,
`medium`, and `high`. Older Hermes versions hardcode `gpt-image-2` in this
