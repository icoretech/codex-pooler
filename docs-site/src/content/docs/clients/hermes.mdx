...
- In Hermes, declare a named provider under `providers:` with your Pooler `base_url` and `extra_body: {service_tier: priority}`, then point `model.provider` at it. Hermes merges `extra_body` into every request it sends to that provider. Check the rest of your setup after you switch providers, because a named provider can resolve its API mode and settings differently from `openai-api`.
