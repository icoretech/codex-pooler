...
Current Codex Pooler releases expose an SDK-readable `context_length` on `/v1/models`, derived from effective Codex `context_window` metadata. OpenAI's GPT-5.6 catalog defaults to 272000 tokens and permits a native 872000-token opt-in; Pooler advertises `floor(872000 × 0.95) = 828400` to `/v1` clients. Keep `context_length: 828400` as the explicit fallback when Hermes cannot read `/v1/models` first; do not configure the raw ceiling. `compression.threshold: 0.95` starts Hermes compression at 786980 tokens. Hermes context compression uses its own auxiliary request timeout. Keep `auxiliary.compression.timeout: 900` so large retained contexts can finish instead of cycling through the older 120-second compression budget. This is independent from the optional MCP server `timeout` and from an application output cap.
