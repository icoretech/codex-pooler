...
Cline's user-facing model metadata names are `contextWindow`, `maxInputTokens`, and `maxTokens`. If you add a manual Codex Pooler model entry in Cline settings, use `contextWindow: 258400`, `maxInputTokens: 130400`, and `maxTokens: 128000` for `gpt-5.6-terra`; Cline's compaction trigger then leaves room for a long completion inside the 258.4k Pooler window. When available, `/v1/models.context_length` is the authoritative effective value for `contextWindow`.
