...
Errors that Codex Pooler itself authors carry an `error.type` that says whether
the failure is worth retrying. The same classification applies to HTTP error
bodies on `/v1/*` and `/backend-api/codex/*` and to error frames on
`GET /v1/responses`, so one code has one type on every surface.
...
string, as described below. The status agrees with the type: a `5xx` response
or frame is never typed `invalid_request_error`. Errors relayed from the Codex
backend follow the rules in the next section.
