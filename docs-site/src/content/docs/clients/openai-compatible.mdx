...
`name` and `namespace` metadata. Omitted or `null` values are accepted; blank or
non-string values are rejected before dispatch. This applies to both `output` and
legacy `result` items over HTTP and the narrow Responses websocket bridge.
...
The request must contain visible input followed by exactly one final
`{"type":"compaction_trigger"}` item. Codex Pooler dispatches it through the
buffered compact bridge, returns the compacted item as collected JSON, public
SSE, or narrow Responses websocket events, and keeps prompt and encrypted
content out of durable metadata. Non-terminal, duplicate, hidden-only, or
otherwise malformed triggers return a sanitized `400` before upstream dispatch.
This does not enable direct `POST /v1/responses/compact`.

On a narrow Responses websocket turn, the trigger first retains the normal
`proxy_websocket` admission, then receives nested `proxy_compact` admission
before compact execution. Local websocket completion returns the same compact
terminal item as the HTTP JSON and SSE surfaces. This does not change legacy
Codex backend compaction behavior.

### Replay a remote-compaction item

When a remote compaction response supplies an encrypted `type: "compaction"`
output item, submit that item unchanged at the start of the next
`POST /v1/responses` input, followed by the new user item. Start a new chain:
omit `previous_response_id` rather than using the compact response id. Codex
Pooler preserves accepted compaction items and their input order over collected
JSON, public SSE, and narrow Responses websocket turns.

The stable public replay fields are `type`, a nonblank opaque
`encrypted_content` string, and optional `id`. The `id` key may be absent, a
string, or explicitly `null`; key presence is preserved exactly. Each public
terminal carries the identical normalized item. Native metadata and unknown
fields are stripped from public compact output, while malformed upstream compact
output returns a sanitized `502`. Unknown replay fields or malformed values are
rejected before upstream dispatch. Treat encrypted content as opaque and do not
log or persist it in application telemetry.

This replay path is separate from the explicit terminal-trigger bridge.
...
On narrow `GET /v1/responses` websocket compatibility, malformed JSON and JSON values that are not objects are ignored without consuming a sequence number or creating a local terminal. This applies to direct and accepted owner-forwarded public clients; native backend websocket behavior is unchanged.
...
No. `POST /v1/responses/compact` returns a deterministic OpenAI-shaped `unsupported_endpoint` error. Explicit compaction through selected provider clients is supported on normal `/v1/responses` requests when the client appends exactly one final `compaction_trigger` after visible input.

Codex backend clients that send the same terminal trigger to `POST /backend-api/codex/responses` or `/backend-api/codex/v1/responses` retain that final item and use the normal backend Responses upstream endpoint. The request remains compact-accounted and buffered through the compact transport, then receives canonical backend Responses SSE compaction output. Direct backend compact aliases remain supported and use the canonical legacy `/backend-api/codex/responses/compact` upstream endpoint.
