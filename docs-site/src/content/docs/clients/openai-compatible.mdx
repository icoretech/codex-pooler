...
`x-should-retry: false`, because the request can succeed then. When
...
same request to another eligible account.
...
message, `resets_at` and `resets_in_seconds` from the backend's reset,
`Retry-After`, and `x-should-retry: false` when the wait is longer than 60
seconds. The backend's own message and other fields are not relayed. A
usage-limit refusal that names no reset still ahead keeps the redacted
`rate_limit_error` described below.
