...
shape with `providerOptions.openai.compactionTrigger: true`:
...
The stable public replay fields are `type`, a nonblank string
`encrypted_content`, and optional `id`. The `id` key may be absent, a string, or
explicitly `null`; key presence is preserved exactly. Unknown compaction fields
or malformed values are rejected before upstream dispatch. Treat encrypted
content as opaque and do not log or persist it in application telemetry.
