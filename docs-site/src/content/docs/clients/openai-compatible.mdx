...
For public SSE, an absent, blank, or whitespace-only `event:` label is treated as absent before the label is compared with the JSON `type`. A nonblank mismatch is still rejected. If an incomplete `POST /v1/responses` SSE block grows beyond 65,536 bytes, Codex Pooler sends one bounded sanitized `type: "error"` event, does not relay that source block, and drops later source frames.
