...
If compression is enabled but savings do not appear, check the request log's `payload_compression` status and reason. The UI prefers saved token count and token savings percent when local token counts are available, then falls back to saved bytes and byte savings percent. It may also show compression processing throughput derived from sanitized original-token count and elapsed time.
