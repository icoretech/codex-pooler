...
Codex Pooler validates and forwards these OpenAI cache controls independently
from Pool routing affinity. GPT-5.6 model ids are passed through to the Pool
catalog and assignment policy; naming one here does not guarantee that every
Pool has an eligible account for that model.
