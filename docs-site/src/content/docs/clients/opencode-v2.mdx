...
OpenCode v2 is an open-source coding agent for working on projects from the terminal. Connect it to Codex Pooler to use your Pool's models while OpenCode reads files, runs tools and maintains your coding sessions. This guide covers v2; use the [OpenCode v1 guide](/clients/opencode/) for the earlier configuration format and OMO integration.
...
Merge this native v2 configuration into `opencode.jsonc` at the path for your system. The custom provider name keeps this connection separate from personal OpenAI credentials.
...
```powershell
Invoke-RestMethod -Uri "https://codex-pooler.example.com/v1/models" -Headers @{ Authorization = "Bearer $env:CODEX_POOLER_API_KEY" }
```
