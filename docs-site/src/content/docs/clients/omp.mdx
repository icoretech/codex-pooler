...
`remoteCompaction.v2StreamingEnabled: true` lets OMP use the Codex-style streaming compaction path. OMP sends a normal backend Responses request with a terminal `compaction_trigger` to `remoteCompaction.v2Endpoint`; Codex Pooler recognizes OMP's V2 compaction metadata, retains `stream: true`, forces `store: false`, and collects the upstream SSE into one replay-safe compaction item before returning the canonical completion. The request remains compact-accounted through the compact transport. Other backend terminal-trigger compactions retain the buffered JSON projection. If OMP falls back, its native request uses `/backend-api/codex/responses/compact`; that native projection omits `store`, `stream`, and the trigger. Treat a native `404` as an unsupported provider capability only after Pooler dispatch evidence shows an admitted upstream attempt, not merely a local route miss.
