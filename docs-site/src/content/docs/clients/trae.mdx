...
Trae combines an AI code editor with agent chat to help you understand and change a project. Add Codex Pooler as a custom model connection to use models available to your Pool. This guide covers the model settings for both Trae and Trae CN.

## Before you start

- Install [Trae](https://www.trae.ai/download) or your regional Trae CN edition and sign in.
...
| 完整 URL | Off |
| 模型 ID | `gpt-6-sol` or another model id served by the assigned Pool |
| API 密钥 | Pool API key |
| 模型系列 | Default |
| 显示名称 | `GPT-6 Sol via Codex Pooler` |
| 上下文窗口 input | `184000` |
| 上下文窗口 output | `16000` |
| Tool Call Rounds | `200` |
...
```powershell
$headers = @{ Authorization = "Bearer $env:CODEX_POOLER_API_KEY" }
$body = @{
  model = "gpt-6-sol"
  messages = @(@{ role = "user"; content = "Reply with exactly: trae ok" })
  stream = $false
  max_completion_tokens = 16
} | ConvertTo-Json -Depth 5
Invoke-RestMethod -Method Post -Uri "https://codex-pooler.example.com/v1/chat/completions" -Headers $headers -ContentType "application/json" -Body $body
```
