...
| `string_above_max_length` in an existing chat | This can refer to a tool-call identifier even when the whole request is small. Pooler shortens oversized Chat call identifiers consistently across calls and results, and splits oversized text without truncation. If the error persists, include your Pooler version and the failed attempt's **Provider rejection parameter** from Request logs. |
