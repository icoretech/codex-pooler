...
Continue uses `contextLength` for request pruning and `defaultCompletionOptions.maxTokens` for the completion budget. It prunes request messages before sending instead of doing Codex-style local compaction, so keep the context length at Codex Pooler's exposed model window. When available, `/v1/models.context_length` is the authoritative effective value for `contextLength`; the 128000-token completion cap leaves 130400 tokens for input.
