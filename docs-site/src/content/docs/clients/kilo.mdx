...
Kilo uses OpenCode-style `limit.{context,input,output}` fields but includes reasoning tokens in overflow accounting and supports `compaction.threshold_percent` for preflight compaction. `limit.input: 258400`, `compaction.reserved: 12920`, and `threshold_percent: 95` make both safety checks meet at 245480 tokens, 95% of the effective window. `limit.input` is the local pre-compaction boundary, not a simultaneous input-plus-output envelope. When available, `/v1/models.context_length` is the authoritative effective value for `limit.context`.
