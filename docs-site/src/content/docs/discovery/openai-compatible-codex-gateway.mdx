...
description: Public answer page for using Codex Pooler's bounded OpenAI-compatible gemma3 facade over account Pools.
sidebar:
  hidden: true
head:
  - tag: script
    ...
    content: |
      {"@context":"https://schema.org","@type":"TechArticle","headline":"OpenAI-Compatible Codex Gateway","description":"Codex Pooler provides bounded OpenAI-compatible routes with one public gemma3 model and Pool API-key routing.","url":"https://docs.codex-pooler.com/discovery/openai-compatible-codex-gateway/","about":["OpenAI-compatible facade","gemma3","Codex gateway","Pool API keys"]}
---

Codex Pooler provides bounded OpenAI-compatible routes for selected SDK clients. Clients call `/v1` with a Pool API key and configure only `gemma3`; Codex Pooler translates supported requests before applying its fixed server policy, Pool routing, limits, account selection, and accounting path.
...
Unsupported `/v1` routes may return deterministic OpenAI-shaped unsupported endpoint errors when explicitly routed. Examples include `POST /v1/content_provenance_checks`, embeddings, batches, moderations, fine-tuning jobs, image variations, response retrieve, response cancel, response delete, and `/v1/responses/compact`.
