...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td><code>https://codex-pooler.example.com</code></td>
    </tr>
    <tr>
      <td>Claude Code or Anthropic Messages client</td>
      <td><code>http://localhost:4000</code></td>
      <td><code>https://codex-pooler.example.com</code></td>
    </tr>
    <tr>
    ...
    <tr>
      <td>Bounded OpenAI-compatible SDK client</td>
...
Every client model field is `gemma3`. Use a Pool API key for `/api`, `/v1`, and `/backend-api`; Anthropic clients may carry that key through their standard auth variables. Use `Authorization: Bearer <operator-mcp-token>` only for `/mcp`.
...
The Codex backend compatibility route is rooted at `/backend-api/codex/*`. The instance root also serves the Ollama `/api/*` and Anthropic Messages `/v1/messages*` adapters, while the remaining documented `/v1` routes provide bounded OpenAI compatibility. Every client configures only `gemma3`; supported reasoning work follows the same fixed server policy, Pool routing, and accounting path.
