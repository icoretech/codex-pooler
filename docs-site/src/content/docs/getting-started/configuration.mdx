...
HTTP requests that continue a Codex session hold a short owner lease on that session. The lease is renewed while the request runs. `bridge_owner_lease_ttl_seconds` is how long a lease stays valid without renewal, and `bridge_owner_lease_renewal_seconds` is how often an active owner renews it. Keep the renewal interval well below the TTL (the defaults renew every `15` s). Change both on the System page, Gateway tab, in the Continuity group. The TTL defaults to `45` s and accepts `24` s or more.

The minimum follows from the gateway's own budgets, not from any installation's latency. A request renews its lease once more just before it reserves capacity and dispatches upstream. If the lease expired during the work before that point, the request is refused with `503` `owner_unavailable`, because an expired lease may already belong to another replica. The TTL must therefore outlast one database statement at its full 15-second budget plus that final renewal.

A value saved before this minimum existed is not rewritten. The gateway uses the minimum in its place and logs `instance setting clamped at read setting=bridge_owner_lease_ttl_seconds` once per replica. Save a value of `24` or more to clear it. Work before dispatch that runs longer than the TTL still ends in that `503`. Clients that retry server errors recover: the retry starts a new session under the same session key.
