...
### Runtime ingress limits and trusted proxies

The runtime firewall applies to the runtime API families and the operator MCP
endpoint. An empty client allowlist disables it. When the allowlist is set, use
exact client IPs or CIDRs. Configure `trusted_proxies` with only the immediate
proxy IPs or CIDRs that are allowed to supply forwarded client headers.

Forwarded headers are resolved conservatively:

- An untrusted immediate peer cannot change the client address; its
  `x-forwarded-for` and `x-real-ip` values are ignored.
- `x-forwarded-for` takes precedence over `x-real-ip`. Duplicate
  `x-forwarded-for` headers are read in wire order and traversed from right to
  left, stopping at the first nontrusted hop.
- Only the first `x-real-ip` value is used when no `x-forwarded-for` header is
  present. A malformed `x-forwarded-for` chain never falls back to it.
- Resolution accepts IPv4, IPv6, bracketed IPv6, and optional decimal ports from
  `1` through `65535`. Forwarded-header values are scanned within a 128-byte
  bound; each trimmed entry is printable ASCII, capped at 64 bytes, and the
  chain is limited to 32 hops.
