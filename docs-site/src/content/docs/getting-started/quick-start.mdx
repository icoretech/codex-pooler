...
head:
  - tag: script
    ...
    content: |
      {"@context":"https://schema.org","@type":"HowTo","name":"Start Codex Pooler locally","description":"Start Codex Pooler, add authorized upstream capacity, create a Pool API key, and configure a supported client with gemma3.","tool":[{"@type":"HowToTool","name":"Docker with Compose"},{"@type":"HowToTool","name":"Git"},{"@type":"HowToTool","name":"openssl"}],"step":[{"@type":"HowToStep","name":"Start Codex Pooler","text":"Clone the repository, generate local secrets, pull images, and start Docker Compose."},{"@type":"HowToStep","name":"Create the owner account","text":"Open http://localhost:4000 and complete bootstrap."},{"@type":"HowToStep","name":"Create a Pool","text":"Create the Pool that clients authenticate against."},{"@type":"HowToStep","name":"Add an upstream account","text":"Link or import an authorized Codex account and assign it to the Pool."},{"@type":"HowToStep","name":"Create a Pool API key","text":"Create a key, copy it once, and keep it in the client environment."},{"@type":"HowToStep","name":"Point a client at the gateway","text":"Choose the documented Ollama, Anthropic Messages, OpenAI, or Codex base URL and configure only gemma3."}]}
---

Use this guide to get from a fresh checkout to a local Pool API key you can put in an Ollama, Claude Code, OpenAI-compatible, or Codex client.

## Quick Setup Summary

To start Codex Pooler locally, generate a private `.env`, start Docker Compose, create the first owner and Pool, assign one upstream account, create a Pool API key, and configure a supported client with `gemma3`. Use `/mcp` only for operator metadata with a separate MCP token.
...
Choose the base URL for the client protocol and configure only `gemma3`:

| Client | Local base URL | Credential |
| --- | --- | --- |
| Ollama | `http://localhost:4000` | Pool API key bearer |
| Claude Code / Anthropic | `http://localhost:4000` | Pool API key through Anthropic auth variables |
| OpenAI SDK | `http://localhost:4000/v1` | Pool API key bearer |
| Codex CLI | `http://localhost:4000/backend-api/codex` | Pool API key bearer |

The server owns the target and fixed reasoning policy. Do not configure another
model, fallback model, or upstream account credential.
...
If the app is running but client requests fail, check the Pool assignment first: the Pool needs an active API key and at least one usable assignment that can route the server-owned fixed target.
