...
10. target serving mode, which chooses Auto, Lite, or Full backend Responses behavior without changing the public `gemma3` identity
...
`Routing strategy` chooses how eligible upstreams are ordered. Strategy does not bypass eligibility. An upstream must still be active, assigned to the Pool, compatible with the fixed target and route shape, and usable according to quota and health evidence.
...
The edit-only `Models` step controls serving mode for real operator-side catalog targets. A saved choice belongs to one Pool-model pair, while clients continue to see only `gemma3`.
...
Codex, Claude Code, Ollama, OpenCode, and other supported clients keep their Pool API key, protocol base URL, and `gemma3` model id. Codex Pooler changes only operator-side target behavior. `GET /v1/models` remains `gemma3`-only and does not expose serving-mode fields; public `POST /v1/responses/compact` remains unsupported.

One HTTP request or one websocket `response.create` turn keeps the mode it started with across retries, assignment failover, and cross-node owner forwarding. A later websocket turn can pick up a saved change. Mode selection does not remove otherwise eligible assignments from the candidate set.

Full never silently downgrades. A generic ordinary Responses HTTP failure under the explicit Full override keeps the upstream status but returns only a fixed server-owned error; provider message, body, code, param, and extra fields are not returned. Request logs classify a non-rate-limit 4xx as `full_upstream_rejection`, a 429 as `upstream_rate_limited`, and an ordinary 5xx as `upstream_status`, alongside the configured/effective/source mode labels. They do not retain the upstream error text, request payload, prompt, response body, or credentials. Auto, Lite, compact or unrelated routes, and established model-unavailability responses keep their existing response behavior. Review provider support, then explicitly select `Auto` or `Lite` and retry as a new request or turn.
...
5. confirm the fixed target and route shape are supported by assigned upstreams
