...
Eligible families are backend Responses, backend `/v1/responses`, backend and public translated chat completions, public `/v1/responses`, backend compact routes, and websocket `response.create` dispatches. Multipart, file, audio, image, admin, MCP, usage, and control-plane requests are not eligible. Public `/v1/responses/compact` remains unsupported because it has no upstream compact dispatch.
