...
Eligible families are backend Responses, compact-accounted terminal-trigger backend Responses dispatches, backend `/v1/responses`, backend and public translated chat completions, public `/v1/responses`, direct backend compact routes, and websocket `response.create` dispatches. Terminal triggers use the normal backend Responses upstream endpoint while retaining their compact accounting and transport identity; direct compact routes use the canonical legacy compact upstream endpoint. Multipart, file, audio, image, admin, MCP, usage, and other non-Responses requests are not eligible. Public `/v1/responses/compact` remains unsupported because it has no upstream compact dispatch.
...
Terminal-trigger requests on either backend Responses route retain the final `compaction_trigger`, dispatch through the normal backend Responses upstream endpoint, and preserve their compact accounting and transport identity. Direct backend compact aliases remain separate and dispatch through the canonical legacy `/backend-api/codex/responses/compact` upstream endpoint.

Backend compact handling uses the same Lite body rewrite as ordinary backend Responses routes before compact-specific projection. In Lite, top-level `tools` and non-blank `instructions` move into `input`; compact-only controls are still removed. In Full, compact keeps its narrow projection without the Lite rewrite.
