...
Assigning the same upstream account to more than one Pool is supported and is often what you want: it pools quota and gives each Pool fallback without a dedicated account. It has one consequence worth knowing before you use Pools as a boundary between groups who should not observe each other.

The provider's prompt-prefix cache is not scoped to the account that warmed it, and it is not scoped to your Pools. Measured against a provider in September 2026: content sent under one upstream account was served back as cached input tokens under a different account, across accounts with separate account ids, separate user ids and no shared workspace. Codex Pooler derives its provider session hint per Pool and per API key, so a hint-based cache hit cannot cross a Pool boundary — but the provider also matches on content alone, which nothing on this side namespaces.

What that means in practice: if two Pools share an upstream account, a client of one Pool that sends a prompt prefix can observe a non-zero cached token count when that exact prefix was sent earlier by a client of the other Pool. It reveals no content and retrieves nothing; it is a confirmation signal that identical bytes were seen before by someone sharing the account.

For a single team or operator this is an efficiency, not a risk, and no action is needed. If your Pools represent genuinely separate tenants who must not learn what the others send, give each tenant its own upstream accounts. No Codex Pooler setting removes this, because the cache lives at the provider.
