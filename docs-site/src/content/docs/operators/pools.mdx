...
The owner `Edit` dialog and the assigned-admin `Models` dialog both use the `Models` panel to control serving mode for each real operator-side catalog target available to the Pool. A saved choice belongs to one Pool-model pair. The same target can therefore use different modes in different Pools, while public clients continue to see only `gemma3`.
...
Backend compact routes use the same Lite body rewrite as ordinary backend Responses routes before compact-specific projection. In Lite, top-level `tools` and non-blank `instructions` move into `input`; compact-only controls are still removed. In Full, compact keeps its narrow projection without the Lite rewrite.
