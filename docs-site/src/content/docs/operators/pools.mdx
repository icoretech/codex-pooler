...
The detail worth knowing is about the provider's prompt-prefix cache, which exists to make repeated prefixes cheaper. That cache matches on content, and it is not partitioned by the account that filled it: measured against a provider in September 2026, content sent under one upstream account came back as cached input tokens under a different account. This is the provider's own behaviour and it applies to anyone sharing an account, with or without Codex Pooler in front of it.
