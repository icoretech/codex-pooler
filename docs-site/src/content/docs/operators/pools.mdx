...
Request logs may show compression savings when safe aggregate data exists. The UI prefers saved token count and token savings percent, then falls back to saved bytes and byte savings percent when token counts are unavailable. When sanitized original-token count and elapsed time are present, the row also derives compression processing throughput without storing a separate throughput value.
