...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>Transport</td>
      <td>Request transport and protocol family, such as HTTP JSON or HTTP SSE. A bolt icon marks a request priced at the priority tier.</td>
    </tr>
    <tr>
      <td>Route</td>
...
When the first reported model differs from the one sent, the list shows a danger triangle and that reported name **beside the requested model**, on the same line. A row without the marker either has matching names or has no recorded name; absence of a warning is not proof of model identity.
