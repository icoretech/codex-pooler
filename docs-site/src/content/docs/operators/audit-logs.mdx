...
2. when a control-plane action happened
...
Use Audit logs with Request logs when routing behavior changes unexpectedly. Request logs show runtime symptoms; Audit logs show whether an operator or system action changed the control plane around the same time.

## Operational Checklist

When investigating a control-plane change, check audit logs in this order:
