...
A Pool API key represents the Pool, not a single upstream. Runtime clients send the key to the documented Ollama, Anthropic Messages, OpenAI-compatible, or Codex backend route, then Codex Pooler applies its fixed `gemma3` facade policy and chooses an eligible upstream using Pool policy, health, limit evidence, and session continuity.
