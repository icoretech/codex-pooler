...
The authenticated admin shell can show one content banner for active incidents reported by the public status feed. The feed is checked every five minutes, and the banner links to the read-only `/admin/incidents` page. It shows up to three incident titles plus an exact `+N more` count and can be dismissed for the current incident revisions by the signed-in operator.

The incidents page preserves the last known good metadata when a refresh fails and labels unavailable or stale data explicitly. The banner is hidden when the last successful refresh is older than 24 hours, and it never appears on the incidents page itself. Resolved incidents stay in bounded history; omission retirement and retention cleanup are handled by scheduled jobs.
