...
A known saved reset expiring within the next 24 hours can be considered without generative or other runtime traffic. Only a canonical scheduled reconciliation for that upstream identity can arm this expiry rescue, and only after it has freshly persisted current quota and saved-reset evidence. Reused, skipped, stale, or otherwise incomplete observations do not arm it. This is the only traffic-independent automatic path: blocked weekly exhaustion and near-limit threshold pressure stay request-driven. Oban deduplicates scheduled work as one incomplete job per upstream identity, then revalidates the active assignment, policy, and fresh evidence before it can consume a credit.
