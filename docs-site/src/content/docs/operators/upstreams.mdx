...
The pause applies to one account's credentials at one provider host. It covers the alternative usage endpoints, the optional reset-credit detail read, the scheduled reconciliation cycle, a manual refresh, and the confirmation read after a saved reset is applied. Every Codex Pooler replica honours the same pause, and it ends on its own when the deadline passes. Codex Pooler never shortens the interval the provider named and has no upper limit for it; only an operator can end a pause early.

While a pause is running, the account card shows `Usage polling paused until` with the deadline, the time left, and what caused it, for example `HTTP 429 with Retry-After`. When more than one usage host of the account is paused, the card shows the latest deadline and how many hosts are paused. The provider host itself is never shown.

`Resume usage polling now` on that notice ends every running pause of the account. It is available to the same operators who can pause and reactivate the account, it asks for confirmation, and it is recorded in the audit log as `Upstream account usage polling pause cleared`, with the number of paused hosts, the latest deadline, and the HTTP statuses that caused them. Nothing else about the account changes: it keeps its status, credentials, Pool assignments and quota evidence, and the next usage read goes out on the ordinary cycle. Resuming overrides the provider's interval by your choice. If the provider still throttles, the next read records a new pause, and that pause is honoured in full, whatever its length. A pause recorded before you resume is removed with it; a shorter instruction arriving while a longer pause runs never shortens it on its own.
...
An account whose usage reads are paused can look like an account whose evidence is simply going stale. Quota evidence stays usable for routing only for a short freshness window after it was observed, so an account that is not serving traffic leaves routing well before a long pause ends, and reconciliation reports that quota refresh is unavailable. Check the account card first: with the pause notice, the cause is the provider's instruction, and you can wait for the deadline or resume usage polling. Without it, treat the stale evidence as an ordinary refresh problem.
