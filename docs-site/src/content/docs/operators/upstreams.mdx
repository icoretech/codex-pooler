...
Click a future expiration countdown in the saved-reset dialog or upstream cockpit, or choose **Download reset calendar** from the cockpit's **Actions**, to download an iCalendar (`.ics`) file containing all reported upcoming banked-reset expirations for that upstream. Each calendar event includes the upstream label, the exact expiration time and a link back to the account. Times are exported in UTC; your calendar displays them in its selected timezone, including daylight-saving changes. Expired and unknown dates are not exported. The file is a snapshot: importing it does not subscribe to updates, and redeeming a reset later does not remove its imported event. The export uses your existing operator access and does not require a Google Calendar or other calendar-provider integration.

Each event is a one-minute calendar marker that starts at the expiration instant. The reset expires at the start of the marker; the displayed duration does not extend its validity.
...
Auto redemption is a per-upstream opt-in policy available from the account-card `Saved resets` dialog and the upstream cockpit. New accounts default to auto redemption off. Operators choose whether request-driven automatic redemption waits until weekly or monthly account quota is blocked or may start earlier when fresh weekly or monthly account quota evidence shows every eligible candidate account is near the configured `Near-limit threshold`. Those are the only immediate request recovery paths: request traffic does not own expiration rescue. The `Natural reset buffer` prevents request-driven exhaustion and threshold recovery from spending a reset when weekly or monthly account quota will reset naturally soon. For traffic-independent scheduled rescue, the ordinary exhausted path and the per-account threshold path also require the configured buffer. A scheduled exhausted path may instead proceed when fresh expiration evidence shows the saved reset expires before the natural long-window reset. `Resets to keep` reserves saved resets from automatic use.
...
Operators can configure the optional `upstream_saved_reset_banked_first_seen` alert rule to notify each time a new saved reset appears for a matching Pool assignment. The evaluator uses a stable v2 upstream-identity key that includes neither `reset_expires_at` nor any expiration instant: saved resets first seen in the same evaluation aggregate into one candidate and one upstream-global incident, with impacted Pool targets aggregated on that incident. A later grant, one first seen after everything an earlier incident reported, opens a new incident and alerts again; the same saved reset never alerts twice. Candidate eligibility uses `first_seen_at`; `granted_at` is display metadata only. The alert uses the same metadata-only observation boundary as the saved-reset UI and does not call provider APIs during evaluation.
...
When every observation of a quota window has gone stale, for example because usage reads keep failing, the card shows the last value reported for the cycle that is still running. An observation whose reset time has already passed describes a cycle that has ended, so it never sets the displayed value while another observation of the same window reports a reset in the future. An observation whose reset time passed more than 30 days ago is ignored by the card, readiness, alerts and routing, and the scheduled runtime cleanup deletes it, so a window the provider no longer reports eventually leaves the card instead of showing its last value indefinitely. Whether the cleanup has already run makes no difference to what Codex Pooler shows or routes. Saved-reset automatic-confirmation state keeps an observation only while that state still refers to a reset less than 30 days old.

### Provider-Requested Polling Pauses

Codex Pooler reads an account's usage endpoints on a short cycle to keep quota evidence fresh. If the provider answers one of those reads with `429 Too Many Requests` or `503 Service Unavailable` and a `Retry-After` header it can act on, Codex Pooler stops reading that account's usage endpoints until the time the provider named.

The pause applies to one provider account at one provider host. It covers the alternative usage endpoints, the optional reset-credit detail read, the scheduled reconciliation cycle, a manual refresh, and the confirmation read after a saved reset is applied. Every Codex Pooler replica honours the same pause. It ends on its own when the deadline passes. The provider throttles the account, not one access token, so the pause survives everything that keeps the credentials on the same provider account: a token refresh, including `Refresh token`, `Pause` followed by `Reactivate`, and a re-import or relink of the same account. It ends early only when the upstream account's stored account id changes to a different provider account; an account imported without an account id keeps the pause only until its credentials are next replaced. Codex Pooler never shortens the interval the provider named, has no upper limit for it, and offers no action to end it early: if the provider is throttling the account, reading again before it asked would bring nothing.

While a pause is running, the account card and the upstream cockpit show `Usage polling paused until` with the deadline, the time left, and what caused it, for example `HTTP 429 with Retry-After`. When more than one usage host of the account is paused, the card shows the latest deadline and how many hosts are paused. The provider host itself is never shown.

A `Retry-After` that pauses usage polling for more than one hour is also written to the application log as a warning (`usage polling paused beyond the long-pause threshold`), with the account id, the status and the deadline, so an unusual interval is noticed even when nobody is looking at the card.

What a pause does not do:

- It is not a quota fact. The account is not marked exhausted, and no reset or window is inferred from it.
- It does not block generation requests. Pool routing keeps using quota evidence that is still fresh, and keeps failing closed when evidence is missing or stale, exactly as it does otherwise.
- It does not spend or re-apply a saved reset. A reset already applied stays pending until valid evidence arrives.
- It does not affect other accounts, or the same account at a different usage host.

If the provider sends no `Retry-After`, or sends one Codex Pooler cannot read, nothing changes: a `429` still tries the alternative endpoint and a `503` still stops that read, both without a pause. A read already in flight when a pause is recorded is allowed to finish.

An account whose usage reads are paused can look like an account whose evidence is simply going stale. Quota evidence stays usable for routing only for a short freshness window after it was observed, so an account that is not serving traffic leaves routing well before a long pause ends, and reconciliation reports that quota refresh is unavailable. Check the account card first: with the pause notice, the cause is the provider's instruction and usage reads resume at the deadline shown. Without it, treat the stale evidence as an ordinary refresh problem.
...
Scheduled proactive refresh is based on token expiry and includes busy accounts.
It is enabled by default; use **System → Gateway → Proactive credential refresh**
to disable this selection. Already queued proactive jobs recheck that setting
before contacting the provider. Manual refresh, lifecycle recovery, and refresh
after an upstream authentication failure remain available.

When proactive refresh starts from an active account and fails transiently,
the same canonically known, unexpired credential remains active and pinned
continuations may keep using it. The failed refresh remains retryable: delays
are 60, 120, 240, 480, 960, 1920, and 3600 seconds before the eighth attempt.
After exhaustion, scheduled selection observes the six-hour cooldown. Unknown
or expired credentials and reactive authentication failures do not receive this
preservation; terminal refresh-token rejection still requires reauthentication.
