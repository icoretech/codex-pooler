...
The account quota percentage always follows the provider's reported usage window while included Codex quota remains. A separate positive credit balance may still appear as a raw balance beside the solid bar, but it does not make the bar striped and is never presented as a fabricated `balance / capacity` total. When the provider reports the included window at 100% and a positive balance remains, the bar becomes striped and continues to show only the current raw credit balance. Provider credit units are not a currency amount, so they should not be compared directly with the monetary balance shown in ChatGPT billing.
