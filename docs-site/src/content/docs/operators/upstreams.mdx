...
An account whose usage reads are paused can look like an account whose evidence is simply going stale. The difference is that the pause is deliberate and self-limiting; if quota evidence for an account has not moved for longer than the provider's stated interval, treat it as an ordinary refresh problem rather than a pause.
