...
When every observation of a quota window has gone stale, for example because usage reads keep failing, the card shows the last value reported for the cycle that is still running. An observation whose reset time has already passed describes a cycle that has ended, so it never sets the displayed value while another observation of the same window reports a reset in the future. The scheduled runtime cleanup removes observations whose reset time passed more than 30 days ago, so a window the provider no longer reports eventually leaves the card instead of showing its last value indefinitely. Observations that carry saved-reset automatic-confirmation state are kept, whatever their age.
