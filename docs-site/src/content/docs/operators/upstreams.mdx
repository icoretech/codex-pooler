...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>Saved resets</td>
      <td>Compact reported saved reset count badge plus bounded post-consume confirmation when a recovery attempt is settling. The badge appears only when at least one saved reset is available; confirmation can remain visible after the count reaches zero. Policy editing opens from the account-card <code>Saved resets</code> dropdown action.</td>
    </tr>
    <tr>
      <td>Pools</td>
...
After a saved reset is consumed, the account card and cockpit can keep a bounded confirmation panel visible even when no saved reset remains in the bank. It separates four outcomes: `Awaiting confirmation`, `Confirmed`, `Not applied`, and `Confirmation expired`. A reset that was accepted but has not yet produced usable quota remains `Awaiting confirmation`; it is not presented as not applied or expired.
