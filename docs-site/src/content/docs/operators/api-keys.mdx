...
For supported Responses WebSocket routes, pausing or revoking a Pool API key blocks new runtime authentication and prompts existing WebSocket connections to close with policy code `1008`. The cluster notification is prompt-only delivery, not the authorization authority.

A durable fence checks the key's persisted status and revocation generation before later work is admitted. If a socket misses or receives a delayed cluster notification, its next frame is still rejected and the connection closes without starting that work.

Only work admitted before the disabling change won the durable ordering may drain and finish once. Queued and later frames do not start after pause or revoke takes effect.

That drain has no deadline of its own. A request already running when the pause or revoke landed finishes under whatever timeout already governs it, and the connection closes when it ends, so the interval is the length of that one request rather than a separate bound you can configure. Everything else stops immediately: the key authenticates nothing new, later frames on an open connection are ignored rather than queued, and work already waiting behind the running request is discarded without being started.
...
Resuming a paused key makes it eligible for new authentication, but it does not reopen an existing socket that was closed or marked for closure. The client must establish a fresh WebSocket connection.
