...
The Models step authorizes the façade's server-owned target. It does not give a client a model picker: every client still sees and sends only `gemma3`.
...
<table>
  ...
  <tbody>
    <tr>
      <td>All models</td>
      <td>Authorizes the fixed target when it is routable, subject to upstream eligibility.</td>
    </tr>
    <tr>
      <td>Selected</td>
      <td>Authorizes the fixed target only when the operator includes its real catalog identifier.</td>
    </tr>
    <tr>
      <td>Deny all</td>
...
Catalog models are real operator-side routing metadata. They are intentionally different from the client-visible virtual model. A Selected policy that omits the fixed target, an enforced model that names anything else, an exact reasoning effort other than `max`, or a maximum reasoning ceiling below `max` conflicts with the façade and fails closed before upstream work.

An unrestricted key authorizes the server-owned decision without restoring client control. `Always use max` is also compatible. Service-tier enforcement remains an independent Pool-key policy. Request logs can show the real target and applied effort to authorized operators, while client catalogs, responses, and errors remain `gemma3`-only.
...
5. confirm model policy authorizes the façade's fixed target and `max` reasoning
