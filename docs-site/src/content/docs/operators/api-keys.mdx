...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>Delete</td>
      <td>Opens a confirmation dialog that requires the key prefix. Delete permanently removes the key and related request history from this instance. Connections still open with the key close as well. See <a href="#responses-websocket-lifecycle">Responses WebSocket lifecycle</a>.</td>
    </tr>
  </tbody>
</table>
...
Delete also removes the key's request history, including the record of a request that is still running. When that record matters, pause or revoke the key first, wait for its connections to close, and delete it afterwards.
