...
An incident is resolved when the condition of the rule that raised it clears. Deleting a rule also removes its targets from its incidents. An incident that no remaining rule targets can no longer clear by itself, so the scheduled evaluation pass, which runs every five minutes, resolves it and records `resolved_reason` `no_rule_target`. This also covers incidents left open by rules deleted with an earlier release. An incident that another rule still targets stays open and keeps following that rule.
...
Quota-state rules judge an account by the quota evidence that can still matter. A rule without a model uses the account windows plus the model windows of models that at least one of the account's Pools currently serves. The last quota observations of a model no Pool serves any more, such as a retired, stale or suppressed catalog model, never make the account stale or exhausted for that rule. The same applies to quota threshold rules without a model: such a model's windows never trigger them. A threshold rule that names a model its Pool does not serve stays clear. A rule that names a model evaluates that model's windows. If the rule's Pool does not serve the model, each assignment is reported as `model_not_served` rather than as stale or exhausted quota. It then counts as not usable for "no usable assignments" rules and never matches a stale or exhausted state rule. Pool incidents list how many assignments were in each state, for example `2 model not served`. The list appears in the incident detail, as `state_counts` in the webhook evidence summary, and in the email evidence summary.

### Saved Reset First-Seen Alerts

The `upstream_saved_reset_banked_first_seen` rule is an optional alert rule that operators configure from the Alerts surface. It is not implicit or always-on. When enabled, it watches persisted saved-reset metadata and alerts once for every saved reset the provider grants to an upstream identity of a matching Pool assignment. Saved resets first seen in the same evaluation aggregate into one upstream candidate and one upstream-global incident.

Evaluation is metadata-only. It reads stored upstream identity metadata and Pool assignments; it does not call provider APIs or refresh account data during alert evaluation. The stable v2 dedupe key is based on upstream identity and includes neither `reset_expires_at` nor an expiration instant. Candidate selection uses `first_seen_at`; `granted_at` is display metadata and does not create or alter an alert.

A saved reset alerts when its `first_seen_at` is later than every saved reset an earlier incident of the same upstream identity already reported. It opens a new incident and is delivered to the linked channels even when the previous incident was resolved long ago, for example after the earlier reset was used or expired. If an earlier incident is still open, the new one supersedes it: the earlier incident is resolved with `superseded_reason` `newer_saved_reset_first_seen`. The same saved reset never alerts twice. It stays quiet while it remains banked, when the incident is resolved by hand, and when it briefly disappears from the provider's inventory and returns with its original first-seen time. Only resets first seen after the rule was created or last enabled can alert. Incidents can name safe labels such as `example-pool` and `example-upstream`, sanitized expiration timestamps, rule kind, severity, and first-seen timing. They must not include raw provider credit objects, request or response bodies, bearer tokens, webhook secrets, prompts, account credentials, or other private content.

If the same upstream identity affects multiple Pools, the single upstream-global incident aggregates impacted Pool targets, and incident visibility follows the current operator authorization model. Owners can see all impacted Pools; assigned admins see only the impacted Pools they are allowed to manage plus safe visible, hidden, and total counts.
