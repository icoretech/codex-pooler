...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Identical. Ordinary Responses work reaches the backend Responses route in both modes</td>
    </tr>
    <tr>
...
On the ordinary Responses lanes — HTTP, SSE, and the websocket `response.create` turn — Lite rewrites the outgoing request body. This is a real translation, not a flag. The backend compact route is a separate compatibility surface and is not described by this table.
...
Codex Pooler applies the Lite translation server-side, at egress, on every lane that dispatches ordinary Responses work. A client that knows nothing about Lite can send an ordinary Full-shaped body and Codex Pooler produces the Lite shape for it.
...
Send `input` as an array of items rather than a bare string on the Codex backend route. Lite assembles its `input` from an item list, and a bare string `input` is not carried into that array, so the turn reaches the provider without that content. The narrow `/v1` surface normalizes a string `input` into an item before the serving mode is applied, so `/v1` clients are unaffected.
...
Because the effective mode is part of the backend catalog body, changing it changes the `GET /backend-api/codex/models` ETag. An open websocket connection keeps the models ETag it handshook with; a new connection observes the updated one.
