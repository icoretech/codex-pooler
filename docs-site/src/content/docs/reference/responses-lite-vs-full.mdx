...
Other anchors are not tied to one upstream connection: a `/v1/responses` tool-output continuation over HTTP, over the streaming bridge or on the public websocket, and a tool-output continuation on the native HTTP route. An OpenAI SDK does not send a request again after `previous_response_not_found`, so these anchors are not refused. Instead, Codex Pooler records on each response of a session the mode it was served in, and a Lite request anchored on a response served in Full carries the tools and instructions it declares at top level as the tools item and the instructions message, which gives the context what it lacks. A request that declares neither, such as the anchored turn of a client that builds Lite-shaped requests itself, gets nothing added. A response to a request that opened no session has no recorded mode: a request opens a session when it sends a session header or `previous_response_id`, so the first continuation of an SDK tool loop that sends no session header is sent as the anchored-request rule above describes.
