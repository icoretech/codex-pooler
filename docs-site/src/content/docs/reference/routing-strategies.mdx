...
`x-session-id` and `x-session-affinity` aren't forwarded upstream. They help Codex Pooler find the local routing state only. On the native `/backend-api/codex/responses` and `/backend-api/codex/responses/compact` routes, the Codex client's own `session-id`, `thread-id`, and `x-client-request-id` headers are forwarded verbatim when they are short ASCII identifiers, because the provider uses them for sticky routing and a full-history HTTP turn without them misses the prompt cache the previous turn warmed. `/v1` routes keep every client continuity header local. When a `/v1/responses` or `/v1/chat/completions` request carries a `prompt_cache_key`, Codex Pooler sends the provider a `session-id` derived from that key (a UUID v5 over a fixed Codex Pooler namespace), so consecutive HTTP turns of one conversation reach the replica that holds the warm prompt cache. The derived value is stable across nodes and restarts, is never stored or logged, and is not sent for keys that are empty, longer than 512 bytes, or not strings.
