...
Early redemption can also run before hard exhaustion when the operator selects near-limit mode and every eligible candidate account has fresh weekly quota evidence at or above the configured threshold. The `Natural reset buffer` applies to threshold mode too, so near-limit auto redemption waits when the natural weekly reset is close. Separately, a known saved reset expiring inside the next 24 hours is the only traffic-independent automatic case: it is evaluated after canonical scheduled account reconciliation has freshly persisted its quota and saved-reset observation, so it does not need generative or other runtime traffic. Blocked weekly exhaustion and near-limit threshold pressure remain request-driven candidate-set paths; scheduled reconciliation never redeems for either condition.
