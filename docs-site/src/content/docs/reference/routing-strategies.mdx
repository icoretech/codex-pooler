...
Early request-driven redemption can also run before hard exhaustion when the operator selects near-limit mode and every eligible candidate account has fresh weekly quota evidence at or above the configured threshold. The `Natural reset buffer` applies to that pool-wide threshold path, so near-limit auto redemption waits when the natural weekly reset is close. Request-driven near-expiry rescue also retains the configured buffer.

Separately, a known saved reset inside the next 24 hours can enter traffic-independent scheduled consideration after canonical account reconciliation persists current quota and saved-reset evidence. Entering that horizon does not consume a reset by itself. Scheduled redemption requires weekly exhaustion, the configured threshold for that identity, or the final inclusive 90-minute last-call window. Scheduled threshold rescue is evaluated per identity; request-driven threshold pressure remains pool-wide. The scheduled exhausted and threshold paths ordinarily require the configured Natural reset buffer. Exhaustion may bypass it when a fresh expiration observation shows expiration before the natural weekly reset; scheduled last-call requires that same fresh expiration-before-reset ordering instead of the full configured buffer.
