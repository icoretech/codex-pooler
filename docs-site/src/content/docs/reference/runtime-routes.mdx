...
`session-id`, `x-session-id`, and `x-session-affinity` are not forwarded upstream. On `/v1/responses` and `/v1/chat/completions`, a `prompt_cache_key` in the body produces a Codex Pooler-derived upstream `session-id` for provider sticky routing; see Routing strategies. If a pinned continuation points at an upstream account that now requires reauthentication, `/v1/responses` HTTP and websocket requests fail closed with a recovery hint to restart with full context and remove stale continuation anchors.
