...
A native HTTP `POST /backend-api/codex/responses` answers such a `400`
refusal (no code, an unknown code, another error type, or a `{"detail": ...}`
body) with the same Codex Pooler-authored error object as JSON, streaming or
