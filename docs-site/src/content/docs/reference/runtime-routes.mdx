...
mismatches remain rejected. An incomplete public Responses SSE block may retain
at most 65,536 bytes. Byte 65,537 emits one bounded sanitized `error` event,
relays none of the overflowing source block, and latches later source frames
out. Complete blocks are still parsed before that overflow decision. A
