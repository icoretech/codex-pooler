...
Request logs show safe aggregate savings only. They prefer saved tokens and token savings percent when token counts are available, and fall back to saved bytes and byte savings percent when token counts are unavailable. Compression processing throughput is derived only from sanitized original-token count and elapsed time.
