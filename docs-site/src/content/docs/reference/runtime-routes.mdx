...
Over HTTP, every other upstream failure keeps its existing behavior: other statuses
(including `401`, `403`, `404`, `422`, `429`, and `5xx`), other error types or
codes, `{"detail": ...}` bodies, and compact routes. The request remains a non-retried failure
...
`422`, and `server_error` for an upstream `401` or `403` (the upstream account's
credentials or standing, not the request), a `429`, a `5xx`, and an upstream
`404`, which `/v1` answers as `502`.
