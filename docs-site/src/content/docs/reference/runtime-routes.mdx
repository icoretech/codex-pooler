...
Reasoning effort values can come from the client request or API key policy. Policy may enforce `none`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`, or `ultra`; `none` is sent as no requested reasoning effort, while an unset policy leaves the client value unchanged. Backend Codex compatibility normalization may rewrite some accepted values for the upstream target, and Request logs expose safe requested, applied, and effective reasoning metadata.
...
and `Edit`, plus external retrieval outputs. Output-only function tool results
fail closed as protected when the tool name is unavailable. Metadata records
aggregate skip counts only.
