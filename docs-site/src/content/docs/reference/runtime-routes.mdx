...
dispatch. When the selected model's catalog lists reasoning levels without
`max`, `ultra` becomes the highest listed level instead, and it never becomes
`none` or `minimal`. `none` and every other explicit effort are forwarded
unchanged, even when the model catalog does not list them, so the upstream
decides whether that model accepts the value. Safe reasoning summaries retain
...
or `null` when upstream supplies no valid path. Chat Completions requests
...
`response_format`, and nested `tools[N].function` fields; other paths stay as
upstream reported them. The message is written by Codex Pooler from the code
...
included, and the list is omitted when it cannot be read safely. A streaming
`POST /backend-api/codex/responses` request receives this JSON body with
`content-type: application/json` and no SSE stream; a non-streaming native
request keeps its existing upstream body.

Every other upstream failure keeps its existing behavior: other statuses
(including `401`, `403`, `404`, `422`, `429`, and `5xx`), other error types or
codes, `{"detail": ...}` bodies, the explicit Full serving-mode override,
compact routes, and websocket turns. The request remains a non-retried failure
recorded as `upstream_status`, and the rejection does not demote the upstream
or affect its circuit.
