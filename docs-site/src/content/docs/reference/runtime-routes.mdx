...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>Supported</td>
      <td>Returns an OpenAI-shaped model list for the authenticated Pool. Includes <code>context_length</code> when Codex context metadata is available; backend-only fields such as <code>context_window</code>, <code>auto_compact_token_limit</code>, and <code>comp_hash</code> are omitted.</td>
    </tr>
    <tr>
...
and long-preamble diffs without a leading `diff --git` header. Malformed NUL
fragments, prose headings, and ordinary prose with plus/minus lines stay
text/skip.
