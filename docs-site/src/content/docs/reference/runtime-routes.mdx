...
kind. This is separate from accepted `custom_tool_call` replay input. Chat
Completions does not accept executable custom definitions or custom choices,
and actual execution availability still depends on the selected model and
upstream account. This narrow contract does not imply broad OpenAI tool parity.
