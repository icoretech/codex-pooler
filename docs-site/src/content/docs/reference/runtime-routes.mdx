...
demote the account: the Codex client's HTTPS fallback is then routed to another
account of the Pool first. A `403` that demotes nothing would only reach the
same account again and is final like the others.
...
(including `401`, `403`, `404`, `422`, `429`, and `5xx`) and compact routes. The request remains a non-retried failure
...
upstream `404`, which `/v1` answers as `502`.
